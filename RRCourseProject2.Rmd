...
We'll tacke this question by grouping by event type and summarizing the injuries column, and order them by number of injuries.

```{r harmfulevents, message=FALSE}
# Group by eventtype, summarize by injuries + fatalities and arrange by injuries + fatalities
harmfulevents <- storms %>% group_by(eventtype) %>% summarise(dead=sum(fatalities), wounded=sum(injuries), deadandwounded=sum(injuries + fatalities)) %>% arrange(desc(deadandwounded))

# print head
head(harmfulevents, 15)
