...
For the purpose of this study, we define 'economic consequences' as any damage inflicted to property and crops. We are not making any distinction as to whether one is more important than the other, neither are we bringing the costs to present value, so the numbers are reported in terms of cost at the time of the inflicted damage.
...
# Top 20 costly events
top20cost <- slice(costlyevents, 1:10)

# make data long and narrow for ggplot2
gatheredcost <- gather(top20cost, type, count, -c(eventtype, totaldamage, propertydamagepct, cropdamagepct))

# rearrange event types to sort them in the plot 
gatheredcost <- mutate(gatheredcost, eventtype=factor(eventtype, levels = top20cost$eventtype))

ggplot(gatheredcost, aes(x=eventtype, y=count/1000000000, fill=type)) +
  geom_bar(stat="identity") +
  theme(axis.text.x=element_text(angle=90, hjust=1)) +
  labs(x='Type of weather event', y='Property + Crop Damage (billions USD)', title='Top 10 weather phenomena most damaging\nto property and crops in the US (1950-2011)')

# Total human cost for tornadoes
totalcosttornadoes <- (filter(top20cost, eventtype=='FLOOD') %>% select(totaldamage))
totalcost <- sum(costlyevents$totaldamage)
