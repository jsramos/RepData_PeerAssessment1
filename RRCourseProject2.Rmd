...
sapply(crude, function(x){sum(is.na(x))})
...
We'll tackle this question first by calculating the total human cost of the entire dataset (all event types and from 1950 to 2011).

```{r totalhumancost, message=FALSE}

# Total human cost in the dataset
totalhumancost <- with(crude, sum(INJURIES+FATALITIES))
print(totalhumancost)
```

We'll then plot the total count of both injured and fatal losses.
...
harmfulevents <- storms %>% group_by(eventtype) %>% 
  summarise(dead=sum(fatalities), deadpct=dead/dandw,  
            wounded=sum(injuries), woundedpct=wounded/dandw, 
            dandw=sum(dead+wounded), dandwpct=dandw/dandw) %>% 
  arrange(desc(dandw))
...
Finally, to summarize these findings, we show a table for these top 8 phenomena, with their total cost, and percent of fatalities and injuries as fractions of total costs.

```{r table, message=FALSE,results='asis'}
tab <- mutate(top20, WeatherPhenomenon=eventtype, TotalHumanCost=dandw, PercentInjured=woundedpct*100, PercentKilled=deadpct*100) %>% arrange(desc(dandw)) %>% slice(1:8)
