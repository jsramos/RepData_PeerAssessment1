...
```
From the graph we conclude that the measured physical activity starts at around the 530 mark, which roughly corresponds to 5:30am, and it peaks at interval **`r maxinterval$interval`**, which is around 8:30am mark with **`r maxinterval$meansteps`** steps. This could mean, if you wish to further hypothesize, that the time of greater activity can be due to members of the study doing their work commute.
...
```{r histogram, message=FALSE,fig.width=10,fig.height=5}
...
```{r differences, message=FALSE, warning=FALSE}
